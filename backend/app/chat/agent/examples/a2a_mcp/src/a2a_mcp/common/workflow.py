...
class WorkflowNode:
    ...
    async def get_planner_resource(self) -> AgentCard | None:
        ...
        try:
            ...
            print("DEBUG: About to create session with init_session")
            async with client.init_session(
                config.host, config.port, config.transport
            ) as session:
                print("DEBUG: Session created successfully, about to find_resource")
                response = await client.find_resource(
                    session, "resource://agent_cards/planner_agent"
                )
                print("DEBUG: Got response from find_resource")
                data = json.loads(response.contents[0].text)
                print("DEBUG: Parsed JSON data")
                result = AgentCard(**data["agent_card"][0])
                print("DEBUG: Created AgentCard successfully: {result.name}")
                print("DEBUG: Agent card url: {result.url}")
                print("DEBUG: Agent card capabilities: {result.capabilities}")
                return result
    ...
    async def find_agent_for_task(self) -> AgentCard | None:
        ...
        if (
            ...
            and self.attributes
            and "agent_type" in self.attributes
        ):
        ...
        try:
            ...
            print(
                f"DEBUG: About to create session for find_agent with query: {query_text}"
            )
            async with client.init_session(
                config.host, config.port, config.transport
            ) as session:
                print("DEBUG: Session created for find_agent, calling find_agent")
                result = await client.find_agent(session, query_text)
                print("DEBUG: Got result from find_agent")
                ...
                if hasattr(result, "content") and result.content:
                    ...
                    if len(result.content) > 0:
                        ...
                        print(
                            f"DEBUG: First content text length: {len(result.content[0].text)}"
                        )

                        if result.content[0].text.strip():
                            try:
                                agent_card_json = json.loads(result.content[0].text)
                                print("DEBUG: Parsed JSON for find_agent successfully")

                                # Check if the response is an error
                                if (
                                    isinstance(agent_card_json, dict)
                                    and "error" in agent_card_json
                                ):
                                    error_msg = agent_card_json["error"]
                                    print(
                                        f"DEBUG: Error response from find_agent: {error_msg}"
                                    )
                                    raise ValueError(f"MCP server error: {error_msg}")

                                logger.debug(
                                    f"Found agent {agent_card_json} for task {self.task}"
                                )
                                agent_card = AgentCard(**agent_card_json)
                                print(
                                    f"DEBUG: Created AgentCard for find_agent successfully: {agent_card.name}"
                                )
                                print(f"DEBUG: Agent card url: {agent_card.url}")
                                print(
                                    f"DEBUG: Agent card capabilities: {agent_card.capabilities}"
                                )
                                return agent_card
                                ...
                                print(
                                    f"DEBUG: Type error creating AgentCard: {type_error}"
                                )
                                print(f"DEBUG: Agent card JSON: {agent_card_json}")
                                raise
                        else:
                            print("DEBUG: Content text is empty or whitespace only")
                            raise ValueError("Empty response from find_agent tool")
                    else:
                        print("DEBUG: No content items in result")
                        raise ValueError("No content in find_agent result")
                else:
                    print("DEBUG: No content attribute in result")
    ...
    async def run_node(
        ...
        try:
            if self.node_key == "planner":
                print("DEBUG: Getting planner resource")
                agent_card = await self.get_planner_resource()
                print("DEBUG: Got planner resource successfully")
            else:
                print("DEBUG: Finding agent for task")
                agent_card = await self.find_agent_for_task()
                print("DEBUG: Found agent for task successfully")
        except Exception as e:
        ...
        print("DEBUG: About to create httpx client and A2AClient")
        print(
            f"DEBUG: Agent card details - Name: {agent_card.name}, URL: {agent_card.url}"
        )
        async with httpx.AsyncClient() as httpx_client:
            print("DEBUG: Created httpx client, creating A2AClient")
            print(f"DEBUG: Trying to connect to: {agent_card.url}")
            client = A2AClient(httpx_client, agent_card)
            print("DEBUG: Created A2AClient successfully")
            ...
            print("DEBUG: Created payload, creating request")
            request = SendStreamingMessageRequest(
                id=str(uuid4()), params=MessageSendParams(**payload)
            )
            print("DEBUG: Created request, starting stream")
            response_stream = client.send_message_streaming(request)
            print("DEBUG: Got response stream, starting iteration")
            async for chunk in response_stream:
                print("DEBUG: Got chunk in response stream")
